...
public class AirPollutionJavaApp {
	...
	public static void main(String[] args) throws IOException {
		...
		url += "&sidoName=" + URLEncoder.encode("서울", "UTF-8"); // 요청 시 전달값 중 한글이 있을경우 Encoding 해야함
		url += "&returnType=json";
		...
		// 인풋스트림 => 바이트 스트림(1바이트씩 읽음) // 리더스트림 => 문자스트림(2바이트씩 읽음)
		...
		// JsonObject, JsonArray, JsonElement 이용해서 파싱할수 있음
		// JSONObject, JSONArray => JSON
		// JsonObject, JsonArray, JsonElement => GSON
		// 각각의 item 정보를 => AirVo 객체에 담고 => ArrayList에 쌓기
		// 우리가 일반적으로 썼던거는 json 라이브러리 / 이거는 gson라이브러리
		
		// 파싱하려면 jsonObject 형으로 만들어야함
		...
		for(int i = 0; i < itemArr.size(); i++) {
			...
			air.setO3Vlaue(item.get("o3Value").getAsString());
